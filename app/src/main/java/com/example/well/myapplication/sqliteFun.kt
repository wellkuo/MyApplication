...
class ItemDAO(context: Context) {
    ...
    // 資料庫物件
    private val db: SQLiteDatabase = sqliteMod.getDatabase(context)
    ...
    fun createSampleData() {
        val item = Item(0, Date().time,  "關於Android Tutorial的事情.", "Hello content", "", "", 0.0, 0.0, 0)
        val item2 = Item(0, Date().time,  "一隻非常可愛的小狗狗!", "她的名字叫「大熱狗」，又叫\n作「奶嘴」，是一隻非常可愛\n的小狗。", "", "", 25.04719, 121.516981, 0)
        val item3 = Item(0, Date().time,  "一首非常好聽的音樂！", "Hello content", "", "", 0.0, 0.0, 0)
        val item4 = Item(0, Date().time,  "儲存在資料庫的資料", "Hello content", "", "", 0.0, 0.0, 0)
